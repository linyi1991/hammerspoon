...
local mod = {}
local emergencyStop  -- [新增] 前置宣告，供看門狗等閉包安全呼叫
...
-- [新增] 累積時間上限控制
-- =======================
-- 累積執行時間上限（秒）。達上限即強制停止所有回圈與操作；0=不限制。
local RUNTIME_LIMIT_SECONDS  = 0  -- 測試可設 1~5
-- 看門狗檢查頻率（秒）
local RUNTIME_CHECK_INTERVAL = 1
...
-- [新增] 累積時間/看門狗狀態
local totalAccumulatedSec = 0
local currentRunStartTs   = nil
local runtimeWatchdog     = nil
local watchdogTripped     = false  -- 避免重複觸發
...
----------------------------------------------------------------------
-- [新增] 累積限制：看門狗（到上限立即強制停止）
----------------------------------------------------------------------
local function startRuntimeWatchdog()
  if runtimeWatchdog and runtimeWatchdog:running() then return end
  if RUNTIME_LIMIT_SECONDS <= 0 then return end
  watchdogTripped = false
  runtimeWatchdog = hs.timer.doEvery(RUNTIME_CHECK_INTERVAL, function()
    local ok, err = pcall(function()
      if watchdogTripped then return end
      local effective = totalAccumulatedSec
      if running and currentRunStartTs then
        local now = hs.timer.secondsSinceEpoch()
        effective = effective + math.max(0, now - currentRunStartTs)
      end
      if effective >= RUNTIME_LIMIT_SECONDS then
        watchdogTripped = true
        hs.alert.show("[auto_prayer] 已達累積上限，強制停止")
        emergencyStop()  -- 直接中斷所有回圈/操作
      end
    end)
    if not ok then
      -- 若回呼出錯，保險起見關閉看門狗，避免 console 洩洪
      if runtimeWatchdog and runtimeWatchdog:running() then runtimeWatchdog:stop() end
      runtimeWatchdog = nil
      hs.alert.show("[auto_prayer] 監視器錯誤，已停止監視")
    end
  end)
end

local function stopRuntimeWatchdog()
  if runtimeWatchdog and runtimeWatchdog:running() then runtimeWatchdog:stop() end
  runtimeWatchdog = nil
end
...
function mod.runOnce()
  ...
  -- [新增] 啟動累積限制監視
  currentRunStartTs = hs.timer.secondsSinceEpoch()
            ...
            pressActionKeysSequentially(1, function()
              ...
              safeAfter(T_WAIT_BEFORE_CLICK, function()
                ...
                safeAfter(T_FINAL_WAIT, function()
                  -- 計入本輪用時
                  if currentRunStartTs then
                    local now = hs.timer.secondsSinceEpoch()
                    totalAccumulatedSec = totalAccumulatedSec + math.max(0, now - currentRunStartTs)
                  end
                  currentRunStartTs = nil
...
function emergencyStop()
  ...
  hs.alert.show("[auto_prayer] 已強制停止（循環關閉）")
end
...
hs.hotkey.bind({"cmd","alt"}, "F6", function()
  ...
  local f = win:frame()
  local mp = hs.mouse.absolutePosition()   -- [修改] 修正 deprecated API
